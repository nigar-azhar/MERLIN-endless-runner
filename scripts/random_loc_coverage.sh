...
for ((counter=1; counter<=30; counter++)); do
  ...
  coverage run --branch run.py --agent=random  --mode=eval --exp_name=clean --weights_dir="$game_name"-wieghts/"$wieght_file" --game=$game_name --mutant=baseline --tries=50 --score=$cov
  coverage html -i

  # Name of the folder to be created
  run_folder_name=logs/coverage-logs/random/"$game_name"/decision/run_"$counter"
  ...
  coverage run run.py --agent=random --mode=eval --exp_name=clean --weights_dir="$game_name"-wieghts/"$wieght_file" --game=$game_name --mutant=baseline --tries=50 --score=10
  coverage html -i

  # Name of the folder to be created
  run_folder_name=logs/coverage-logs/random/"$game_name"/statement/run_"$counter"

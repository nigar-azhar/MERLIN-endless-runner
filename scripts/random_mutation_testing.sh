...
for ((counter=1; counter<=30; counter++)); do
  ...
  for folder_name in $folder_names; do
      if [[ $folder_name != "baseline" && $folder_name != "__pycache__" ]]; then
  ...
  mv logs/mutation-testing-logs/random/"$game_name"/*.tsv "$run_folder_name"/
  # Move all .csv files to the created folder
  mv logs/mutation-testing-logs/random/"$game_name"/*.csv "$run_folder_name"/

...
DIGITS.append(pygame.image.load('games/flappybird/actual/assets/minus.png').convert_alpha())
...
class Pipe(Sprite):
    ...
    def __init__(self, x_init):
        ...
        self.y_upper_end =  y_upper +220#midpoint - self.gap/2

        #print("Pipe position", (self.x, self.y))
...
class Bird(Sprite):
    ...
    def update(self, key_press=False):
        ...
        #print("bird y", self.y)
        #print("bird x end", self.x+self.image.get_width())
...
class GameText():
    ...
    def draw_score(self):
        ...
        if self.score >=0:
            ...
            #score_digits=["-"]
            score_digits = [int(i) for i in list(str(abs(self.score)))]
            #score_digits.extend(score_digits1)
        ...
        if self.score<0:
            self.surface.blit(DIGITS[10], (x, self.y_score))
            x += DIGITS[10].get_width()
    ...
    def update_score(self):
        ...
        We call this function every time the bird makes
        it through a pair of pipes, so we increment the
        score by 1.
        """
        self.score -= 1

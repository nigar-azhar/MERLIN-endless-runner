...
class Game():
    ...
    def step(self, action):
        ...
        if action and pygame.time.get_ticks() > self.block_action_counter:
            self.block_action_counter = pygame.time.get_ticks() + 1
        ...
        if (self.player.count > self.previous_action_count + 1) or action == 1:
...
def listen(action_block_counter = 0):
    ...
    for event in pygame.event.get():
        ...
        if event.type == KEYDOWN and event.key == K_SPACE and pygame.time.get_ticks() > action_block_counter:
            flap()
            action_block_counter = pygame.time.get_ticks() + 1

...
class Game():
    ...
    def get_game_state(self):
        ...
        topPipe = {'yCoordinate': self.pipes[0].y_upper_end, 'xCoordinate': self.pipes[0].x,
                   'isActive': not self.heartboosterguage.active}
        bottomPipe = {'yCoordinate': self.pipes[0].y_upper_end + 158, 'xCoordinate': self.pipes[0].x,
                      'isActive': not self.heartboosterguage.active}
        ...
        heart = {'yCoordinate': self.heart.y, 'xCoordinate': self.heart.x,
                 'timeElapsed': max(self.last_heart_spawn_time, 0), 'isActive': self.heartboosterguage.active}
    ...
    def step(self, action):
        ...
        if self.game_text.score > 5 and self.heart.x < 0 and self.last_heart_spawn_time < 0 and not self.game_freeze:  # and random.randint(0, 10) == 1:
        ...
        if self.game_text.score > 5 and self.coin.x < 0 and self.last_coin_spawn_time < 0 and not self.game_freeze:  # and random.randint(0, 10) == 1:
            # print("I am spawning heart")

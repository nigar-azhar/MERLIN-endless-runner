...
def verify_state(gstate, previousState):
    ball, bar, collectable = gstate['ball'], gstate['bar'], gstate['collectable']
    bar = dotdict(bar)
    ...
    for state in allstates:
        ...
        if state.find('constraint') is not None:
            ...
            result = eval(constraint_test, {'ball': ball,
...
class RewardEstimator:
    ...
    def estimate(self, gstate, done, action=0):
        ...
        if done:
            ...
            reward = 2
        elif cstero == "good" and pstero == "dead":
            reward = 2

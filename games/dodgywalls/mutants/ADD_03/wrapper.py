...
class Game():
    ...
    def __init__(self, frame_size=84, width=288, height=512):
        ...
        self.block_action_counter = pygame.time.get_ticks() #+ self.fps
    ...
    def step(self, action):
        ...
        if action == 1 and pygame.time.get_ticks() > self.block_action_counter:
            if not self.clicked:
                self.clicked = True

            self.block_action_counter = pygame.time.get_ticks() + (self.fps * 3)
